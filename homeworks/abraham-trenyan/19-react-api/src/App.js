...
import './App.css';
import buttonsList from '../src/utils/buttons.js'
import Container from './components/Container/Container';
import Screen from './components/Screen/Screen';
import Keyboard from './components/Keyboard/Keyboard';
import Button from './components/Button/Button';
function App() {
  const [buttons] = useState(buttonsList);
  ...
  const handlePercent = (calc) => {
    ...
    if (!lastSignIndex || percentIndex <= lastSignIndex) {
      ...
      percentOperation = [
        multiplier,
        percentOperation[lastSignIndex],
        (Number(numberToDivideByHundred) / 100) * multiplier,
      ].join('');
